...
theme.inject_theme()
st.markdown(theme.banner_html(), unsafe_allow_html=True)
...
if st.button("Ask", type="primary") and user_question:
    with st.spinner("Running forecast, retrieving RBI context, and generating explanation..."):
        try:
            ...
            if retrieval_mode.startswith("n/a"):
                mode_label = "💬 General knowledge (no forecast/RBI data needed)"
            else:
                mode_label = "🧠 Semantic retrieval" if retrieval_mode == "semantic" else "🔤 Keyword (TF-IDF) retrieval"
            badges = theme.chip_html(mode_label)

            if report is None:
                pass  # general question -- nothing to ground-check against
            elif report.overall_flag == "grounded":
