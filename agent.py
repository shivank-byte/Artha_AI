...
GENERAL_QUESTION_SYSTEM_PROMPT = """You are an economics assistant. Answer the user's question \
directly and simply in 1-3 sentences, using standard, well-established economic knowledge. \
Do not mention forecasts, RBI, or any "provided information" -- there is none here, just answer \
the question on its own terms."""

GUARDRAIL_SYSTEM_PROMPT = """You are an economic assistant explaining India's current inflation \
forecast and RBI's current policy stance to a general audience.
...
def is_general_question(question: str) -> bool:
    """Heuristic: does this question need the current forecast/RBI data at
    all, or is it a general/definitional question answerable from
    standard economic knowledge alone? Checked BEFORE calling the LLM --
    more reliable than asking the model to ignore irrelevant context
    handed to it, which in practice it doesn't always do well."""
    q = question.lower()
    specific_signals = [
        "rbi", "forecast", "current", "latest", "today", "now", "2025", "2026",
        "repo rate", "policy", "rising", "outlook", "trend", "expect", "will inflation",
        "next month", "this month", "recent",
    ]
    if any(sig in q for sig in specific_signals):
        return False
    general_starters = ["what is", "what's", "define", "explain", "meaning of", "how does", "why does", "why is"]
    return any(q.strip().startswith(s) or s in q for s in general_starters)
...
def build_grounded_prompt(forecast_summary: str, retrieved_chunks: list, user_question: str) -> str:
    ...
    Only used for current-forecast/current-policy questions -- see
    is_general_question() and run_advisor()."""
    context_block = "\n\n".join(
...
Answer the user's question. If it's a general/definitional question, answer simply and directly. \
If it's about the current forecast or RBI's current stance, stay strictly within the FORECAST \
DATA and RETRIEVED CONTEXT above."""
...
def run_advisor(user_question: str):
    ...
    Returns (answer: str, groundedness_report: GroundednessReport | None, retrieval_mode: str).

    General/definitional questions (see is_general_question) skip the
    forecast+retrieval entirely and get a plain, simple answer -- handing
    the model irrelevant data and hoping it ignores it proved unreliable
    in practice."""
    if is_general_question(user_question):
        answer = call_llm(GENERAL_QUESTION_SYSTEM_PROMPT, user_question)
        return answer, None, "n/a (general question, no retrieval needed)"
